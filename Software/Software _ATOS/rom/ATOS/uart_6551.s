...
;
; Modified: 2025-11-30 by kayto@github.com for AT65C02
; - Changed UAGET to truly non-blocking (returns 0 if no character)
; - Added separate LAB_WAIT_Rx blocking routine
;
;*** I/O Locations *******************************
...
LAB_WAIT_Tx:
	...
	AND	#$10		; mask transmit buffer status flag (6551 TDRE = bit 4)
...
; Returns: A = character if available, 0 if no character
;
UAGET:
	LDA	ACIA_STATUS		; get ACIA status
	AND	#$08		; mask rx buffer status flag (6551 RDRF = bit 3)
	BEQ	UAGET_NONE	; return 0 if rx buffer empty
...
; blocking get character routine (wait for ACIA and Rx byte)
LAB_WAIT_Rx:
	LDA	ACIA_STATUS		; get ACIA status
	AND	#$08		; mask rx buffer status flag (6551 RDRF = bit 3)
