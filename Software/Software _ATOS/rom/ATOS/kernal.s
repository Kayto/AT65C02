...
GETLIN: CLI
        ...
        STA     STOPFLAG
        STA     ESCFLAG
WTCHR:  JSR     UAGET          ; wait for character from UART
...
IRQ:    PHA
        ...
        BEQ     BRK_LABEL       ; jump if not
        JMP     ($0316)         ; jump to BREAK vector
BRK_LABEL:
        JMP     ($0314)         ; jump to IRQ vector
...
;;; ----------------------------------------------------------------------------
 ...
 .if (UART_TYPE = 6522)
   .include "uart_6522.s"
 .elseif (UART_TYPE = 6551)
   .include "uart_6551.s"
 .elseif (UART_TYPE = 6850)
   .include "uart_6850.s"
 .else
...
;;; Export UART functions for XMODEM use
        .export UAINIT, UAGET, UAPUTW, LAB_WAIT_Rx
