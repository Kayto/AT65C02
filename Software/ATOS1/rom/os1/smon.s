...
LCD72:  ;.if UART_TYPE==6522     ; if VIA is also used as UART
        ...
        lda     #$C0
        sta     VIA1_IER         ; enable VIA timer 1 interrupt
        lda     #$00
        sta     VIA_ACR         ; VIA timer 1 single-shot mode
        ldx     #0
        lda     #73             ; 73 cycles until timer expires
        sta     VIA1_T1LL        ; set VIA timer 1 low-order latch 
        stx     VIA1_T1CH        ; set VIA timer 1 high-order counter (start timer)
        lda     #<TWINT         ; (2)
        ldx     #>TWINT         ; (2)
        sta     $02BB           ; (4)
        stx     $02BA           ; (4)
